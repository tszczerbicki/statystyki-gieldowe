...
``` {python}
# Podsumowanie odstawowe 

g = df.loc[df['in_WIG20']].groupby('YTD_group')['ticker']

summary = g.count().to_frame('count')
summary['pct_of_total'] = round(g.count() / g.count().sum() * 100,2)
...
``` {python}
df.loc[df['in_WIG20']].groupby(['YTD_group','Volume_change_group'], as_index= False)['ticker'].count().pivot_table(index = 'YTD_group', columns = 'Volume_change_group', values = 'ticker', aggfunc="sum",
...
```{python}
df.loc[df['in_WIG20']].groupby(['YTD_group','Drawdown_group'], as_index= False)['ticker'].count().pivot_table(index = 'YTD_group', columns = 'Drawdown_group', values = 'ticker', aggfunc="sum",
                                                                                         margins = True, margins_name = 'Razem')
```
...
Rozdział dotyczy spółek zaliczanych do WIG,z wyłączeniem WIG140.